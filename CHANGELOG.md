...
- RPM packages for amd64 and aarch64, which were already being built but never
  published
- Snap packages for arm64 alongside amd64, now built in CI
- Experimental Flatpak bundle for self-hosted distribution; host commands are
  reached through `flatpak-spawn`

### Fixed

- Connection status polling aborted the whole app when `ip` was unavailable
- Release asset updates failed on any re-run, because assets were deleted under
  a different name than they were uploaded with
